...
export class FileUpload extends React.Component<FileUploadProps, FileUploadState> {
    ...
    componentWillUnmount() {
        if (this.dropRef.current) {
            this.dropRef.current.removeEventListener('dragenter', this.dragEnterHandler);
            this.dropRef.current.removeEventListener('dragover', this.dragOverHandler);
            this.dropRef.current.removeEventListener('dragleave', this.dragLeaveHandler);
    ...
    private dropHandler = (e: DragEvent) => {
        ...
        if (e.dataTransfer.files.length === 1) {
            ...
            failEarlyStateSet(this.state, this.setState.bind(this), 'error')('You can only place one file');
        }
    ...
    render() {
        return (
            <div className="file-upload">
                ...
                <div
                    ref={this.dropRef}
                    className={`dropzone ${this.state.drag ? 'hovering' : ''} ${this.state.chosen ? 'chosen' : ''}`}
                >
                    {
                        this.state.chosen
