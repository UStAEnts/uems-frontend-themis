...
export function Comment(props: CommentPropsType) {
	...
	return (
		<div
			className={
				'comment ' + (props.comment.requiresAttention ? 'attention' : '')
			}
		>
			<div className="left">
				<ReactTooltip place="top" type="dark" effect="float" id={`cb-${id}`}>
					<strong>Category: </strong>
					<span>
						{props.comment.topic ? props.comment.topic.name : 'No Topic'}
					</span>
				</ReactTooltip>
				<div
					data-tip
			...
			<div className="flags">
				...
				>
					Mark as{' '}
					{props.comment.requiresAttention ? 'resolved' : 'requiring attention'}
				</ReactTooltip>
				{props.comment.requiresAttention ? (
					<FontAwesomeIcon
						data-tip
						data-for={`cb-attention-${id}`}
						icon={faCheckCircle}
