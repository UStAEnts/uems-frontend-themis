 ...
 * @constructor
 */
const VenueChip: React.FunctionComponent<VenueChipType> = ({
	venue,
	link,
	children,
	...rest
	...
	const classes = [styles.venueChip, rest.className]
		.filter((e) => e !== undefined)
		.join(' ');

	if (link) {
		return (
			<Link to={`/venues/${venue.id}`} className={classes} style={style}>
				{internal}
			</Link>
		);
	}
 ...
 * @constructor
 */
export const RemovableVenueChip: React.FunctionComponent<
	RemovableVenueChipType
> = (props) => (
