...
class ValueSquareClass extends React.Component<
	ValueSquarePropsType,
	ValueSquareStateType
> {
	...
	private generateHeader = () => {
		if (this.props.icon && this.props.name) {
			return (
				<div
					className="icon"
					style={{ background: this.props.color ?? Theme.NOTICE }}
				>
	...
	render() {
		...
		const content = (
			<div
				...
				style={{
					background: this.props.color ?? Theme.NOTICE,
					...this.props.style,
				}}
			>
				{this.generateHeader()}
				<div className="conts">{value}</div>
				{this.props.link ? (
		...
		return this.props.link ? (
			<Link to={this.props.link}>{content}</Link>
		) : (
			content
		);
	}
