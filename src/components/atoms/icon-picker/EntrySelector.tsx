...
export class EntrySelector extends React.Component<
	EntrySelectorPropsType,
	EntrySelectorStateType
> {
	...
	private search = (input: string): OptionType[] => {
		...
		for (const option of this.props.options) {
			...
			for (const token of tokens) {
				...
				if (option.additionalSearch && option.additionalSearch.includes(token))
					score += 1;
			}
	...
	private renderEntry = (option: OptionType, spaceOnSelected?: boolean) => {
		const styleOverrides: CSSProperties | undefined =
			this.state.selected?.identifier === option.identifier
				? {
						background: Theme.NOTICE,
						color: 'white',
						...(spaceOnSelected ? { marginRight: '20px' } : {}),
				  }
				: undefined;
		return (
			<div
	...
	render() {
		...
		return (
			<div
				className={`entry-selector${this.state.active ? ' active' : ''}`}
				ref={this.ref}
			>
				<TextField
