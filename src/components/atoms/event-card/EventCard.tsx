...
> {
	...
	private getTimeDifference() {
		const duration = moment.duration(
			moment
				.unix(this.props.event.end)
				.diff(moment.unix(this.props.event.start))
		);
	...
	private getDurationIcon() {
		const hours =
			Math.abs(this.props.event.end - this.props.event.start) / 36e5;
		if (hours > 6) {
			return (
				<FontAwesomeIcon
	...
	private getEntsState() {
		...
		if (this.props.event.ents !== undefined) {
			if (this.props.event.ents.color !== undefined) {
				...
				status = (
					<div className={`ents-state ${this.props.event.ents.name}`}>
						{this.props.event.ents.name}
					</div>
				);
			}
	...
	private generateBorderColorCSS() {
		const stateColor =
			this.props.event.state === undefined ||
			this.props.event.state.color === undefined
				? '#B1B9B8'
				: this.props.event.state.color;
		const entsColor =
			this.props.event.ents === undefined ||
			this.props.event.ents.color === undefined
				? '#B1B9B8'
	...
	private renderCollapsed() {
		return (
			<div className="event-card collapsed">
				<div className="upper">
					<div className="top-bar collapsed">
						...
						<div className="names">
							...
							{this.props.event.venues.map((e) => e.name)}
				...
				<div
					className="lower" /*style={{ borderColor: this.props.event.color }}*/
				>
				...
				<div
					data-tip
					data-for={`cb-${Buffer.from(this.props.event.name).toString(
						'base64'
					)}`}
					className="color-bar"
				...
				<div className="upper">
					...
					<div className="bottom-bar row">
						...
						<div className="col-xs-6 horizontal">
							...
							<div className="time">
								{moment.unix(this.props.event.start).toISOString()}
							</div>
						</div>
					</div>
				</div>
