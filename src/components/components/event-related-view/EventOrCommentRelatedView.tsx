...
class EventOrCommentRelatedViewClass<T, C> extends React.Component<
	EventRelatedViewPropsType<T, C>,
...
> {
	...
	private getFrequency = (events: EventList) => {
		...
		if (duration.asMinutes() >= 1) {
			return `${EventOrCommentRelatedViewClass.round(
				duration.asMinutes()
			)} mins`;
		}
	...
	private generateText = (
		property: Extract<keyof T, string>,
		type: 'textarea' | 'number' | 'text' = 'text'
	) => (
	...
	private renderCommentRightPane = () => (
		<div className="right-props">
			{!this.props.comments ? undefined : (
				<CommentList comments={this.props.comments} topics={[]} withoutBox />
			)}
		</div>
	);
	...
	private renderEventRightPane = () => {
		...
		if (this.props.events) {
			table = <EventTable events={this.props.events} />;
		} else {
			table = <Loader type="BallTriangle" color={Theme.NOTICE} />;
		}
	...
	private delete = () => {
		...
		Promise.resolve(this.props.delete.onDelete())
			.then(() => {
				failEarlyStateSet(
					this.state,
	...
	render() {
		...
		for (const key of Object.keys(this.props.obj) as Extract<
			keyof T,
			string
		>[]) {
			...
			const override = this.props.configOverrides?.find(
				(e) => e.property === key
			);
			...
			if (type === 'textarea')
				properties.push(this.generateText(key, 'textarea'));
		}

		return (
			<div
				className={`view-venue ${this.props.events ? 'events' : 'comments'}`}
			>
				<EditableProperty
					name="name"
					config={{
...
// @ts-ignore
export const EventOrCommentRelatedView: React.ComponentClass<
	EventRelatedViewPropsType<any, any>,
	// @ts-ignore
	EventRelatedViewStateType
> = withRouter(withNotificationContext(EventOrCommentRelatedViewClass));
