...
const and = (...c: (string | undefined)[]) =>
	c.filter((e) => e !== undefined).join(' ');
const activeIfTrue = (current: string, path: RegExp) =>
	path.test(current) ? classes.active : undefined;

const MdiViewDashboardOutline = () => (
...
const Conditional: React.FunctionComponent<{
	...
	return (
		<div className={and(classes.sidebar, classes.expanded)}>
			<img
				...
				alt={
					'UEMS Logo: the text UEMS in a geometric font at the bottom of a black stroked rectangle'
				}
			/>
			...
			<div className={and(classes.marker, ait(/\/events\/?.*/))}>
				...
				<NavLink
					to="/events/create"
					exact
					className={and(classes.navigationLink, classes.sub)}
					onClick={navLinkClick}
					activeClassName={classes.active}
				>
					<div className={classes.icon}>
