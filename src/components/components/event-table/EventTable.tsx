...
> {
	...
	private filter(event: UEMSEvent) {
		if ('name' in this.state.filters) {
			...
			if (!event.name.toLowerCase().includes(filter.content.toLowerCase()))
				return false;
		}
		...
		if ('state' in this.state.filters) {
			...
			if (filter.selectedOption.value !== 'any') {
				if (
					...
					(filter.selectedOption.additional as State).name.toLowerCase()
				)
					return false;
			}
		}
		...
		if ('ents' in this.state.filters) {
			...
			if (filter.selectedOption.value !== 'any') {
				if (
					...
					(filter.selectedOption.additional as EntState).name.toLowerCase()
				)
					return false;
			}
		}
			...
			if (filter.selectedOption.value !== 'any') {
				if (
					!event.venues
						...
						.includes(
							(filter.selectedOption.additional as Venue).name.toLowerCase()
						)
				)
					return false;
