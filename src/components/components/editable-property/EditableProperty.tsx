...
import {
	TextField,
	TextFieldPropsType,
} from '../../atoms/text-field/TextField';
...
export class EditableProperty extends React.Component<
	EditablePropertyPropsType,
	EditablePropertyStateType
> {
	...
	private saveProperty = () => {
		if (
			this.state.selectProperty === undefined &&
		...
		if (this.props.config.type === 'date' && this.state.selectedTime) {
			if (this.props.config.onChange)
				this.props.config.onChange(this.state.selectedTime);
		}
		if (this.props.config.type === 'text' && this.state.input) {
			if (this.props.config.onChange)
				this.props.config.onChange(this.state.input);
		}
		if (this.props.config.type === 'icon' && this.state.selectProperty) {
			if (this.props.config.onChange)
				this.props.config.onChange(this.state.selectProperty as OptionType);
		}
		if (this.props.config.type === 'color' && this.state.selectProperty) {
			if (this.props.config.onChange)
				this.props.config.onChange(this.state.selectProperty as string);
		}
		if (
			this.props.config.type === 'checkbox' &&
			this.state.input !== undefined
		) {
			if (this.props.config.onChange)
				this.props.config.onChange(this.state.input as boolean);
		}
	};
	...
	private renderCheckbox = (box: CheckboxType) => (
		<>
			...
			/>
			<label htmlFor={`editable-checkbox-${btoa(this.props.name)}`}>
				{this.props.name}
			</label>
		</>
	);
	...
	private renderEditField = () => {
		switch (this.props.config.type) {
			...
			case 'select':
				return this.renderSelect(this.props.config);
			case 'text':
				return (
					<TextField
						...
						<TextField
							style={{
								borderBottom: `5px solid ${
									this.state.selectProperty ?? '#000000'
								}`,
							}}
							...
							name="Color"
							initialContent={
								(this.state.selectProperty as string | undefined) ?? '#000000'
							}
						/>
