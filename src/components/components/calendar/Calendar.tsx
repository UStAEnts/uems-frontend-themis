...
> {
	...
	constructor(props: Readonly<CalendarPropsType>) {
		...
		const bindFunctions: Function[] = [this.renderHeader, this.changeDay];
	...
	private renderHeader() {
		...
		for (let i = 0; i < 7; i++)
			days.push(this.state.startMoment.clone().add(i, 'days'));
	...
	private renderHourBox(time: Moment) {
		...
		return (
			<div className="hour-entry" key={limit.unix()}>
				{events}
			</div>
		);
	}
	...
	private renderHourRow(hour: number, minute: number) {
		...
		return (
			<div key={`hr${hour}.${minute}`} className="hour-row c-row">
				<div className="hour-label">{`${Calendar.padToTwo(
					hour
				)}:${Calendar.padToTwo(minute)}`}</div>
				{entries}
			</div>
		);
	}
	...
	render() {
		return (
			<div className="calendar">
				{this.renderHeader()}
				<div className="hour-sequence">{this.renderHourRows()}</div>
				<button type="button" onClick={() => this.changeDay(-1)}>
					Minus One
				</button>
				<button type="button" onClick={() => this.changeDay(1)}>
					Add One
