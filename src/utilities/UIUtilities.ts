...
import { NotificationContextType, NotificationPropsType } from '../context/NotificationContext';
...
export class UIUtilities {
	...
	static load<T>(
		...
		response.then((result) => {
			...
			if (result.status === 'FAILED') {
				...
				else if (typeof (error) === 'function') errorString = error(result.error);
				...
				if (result.status === 'PARTIAL') UIUtilities.tryShowPartialWarning({ notificationContext: realContext });
	...
	static loadUnwrapped<Z, T extends ({ status: 'OK' | 'PARTIAL', result: Z } | { status: 'FAILED', error: string })>(
		context: NotificationContextType | NotificationPropsType,
		response: Promise<T>,
		error?: string | ((error: string) => string),
	): ({ data: (handler: (data: T) => void) => void }) {
		...
		response.then((result) => {
			...
			if (result.status === 'FAILED' && 'error' in result) {
				let errorString = `There was an error fetching data from the server: ${ (result as any).error }. If you believe you have received this in error, please contact a member of support!`;
				if (typeof (error) === 'string') errorString = error;
				else if (typeof (error) === 'function') errorString = error((result as any).error);
	...
	static async deleteWith409Support(handler: () => Promise<any>): Promise<boolean> {
		try {
			await handler();
			return true;
		} catch (e) {
