...
import SelectPart, {SelectEditPart, SelectOptionConfiguration, SelectValidator} from "./parts/Select";
import TextPart, {TextConfiguration, TextEditPart, TextValidator} from "./parts/Text";
import CheckboxPart, {CheckboxConfiguration, CheckboxEditPart, CheckboxValidator} from "./parts/Checkbox";
import NumberPart, {NumberConfiguration, NumberEditPart, NumberValidator} from "./parts/Number";
import DatePart, {DateConfiguration, DateEditPart, DateValidator} from "./parts/Date";
import DateRangePart, {DateRangeConfiguration, DateRangeEditPart, DateRangeValidator} from "./parts/DateRange";
...
export const getQuestions = (stage: Form['stages'][number]) => 'title' in stage ? stage.questions : stage;
...
export const BOOKING_FORM: Form = {
    stages: [
        ...
        /*1*/[
            ...
            /*1.3*/{
                ...
                options: [
                    ...
                    {value: '30', text: '30 mins - eg small presentations in Beacon / Sandy\'s'},
                    {value: '60', text: '1 hour - eg club night in 601 or movie / presentation in StAge'},
                    {value: '90', text: '1.5 hours - eg full venue club night'},
                    {value: '120', text: '2 hours - eg club night in 601 with decorations'},
                    {value: '150', text: '2.5 hours - eg full venue club night with decorations'},
                    {value: '180', text: '3 hours - eg live music or ceilidh'},
                    {value: '240', text: '4 hours - eg full venue with major decorations'},
                    {value: '300', text: '5 hours - eg dance or variety show'},
                    {value: '720', text: '12 hours - eg major act'},
                    {value: '1560', text: '26-36 hours - eg over multiple days, catwalks'},
        ...
        /*2*/[/*2.0*/{
            type: 'text',
            prompt: 'Setup time required',
            detail: 'You specified "Other" as the setup time required - please describe, in detail, how much time you will need',
            required: true,
        }],
        /*3*/[/*3.0*/{
            type: 'text',
            prompt: 'Event type',
            detail: 'You specified "Other" as the event type - please describe, in detail, your event type',
            required: true,
        }],
        /*4*/[/*4.0*/{
            type: 'text',
    ...
    flow: [
        0,
        1,
        {condition: [{type: 'eq', target: 'event-type', value: 'Other'}], stages: 3},
...
export const FormEdit: React.FunctionComponent<Form & {
    ...
    const changeType = (form: Form, stage: number, index: number, type: string) => {
        const deepCopy = JSON.parse(JSON.stringify(form)) as Form;

        const stageElement = deepCopy.stages[stage];
        const questionArray = 'title' in stageElement ? stageElement.questions : stageElement;
    ...
    const onMove = (form: Form, stage: number, index: number) => ((d: 'up' | 'down') => {
        ...
        deepCopy.stages[stage][index + (d === 'up' ? -1 : 1)] = deepCopy.stages[stage][index];
    ...
    const convertStageToElements = (form: Form, stages: StageType, stage: number, index: number) => {
        switch (stages.type) {
            case 'number':
                return (<NumberEditPart
                    {...stages}
    ...
    const elements = form.stages.map(
        (e, si) => {
...
const FormEntry: React.FunctionComponent<Form & { onFormSubmit: (values: Record<string, any>) => void }> = (props) => {
    ...
    useEffect(() => {
        if (status !== null && ref.current) ref.current.scrollIntoView({behavior: 'smooth'});
    }, [status, ref]);

    // Evaluate flow
    const stages = (props.flow.map((e) => {
        ...
        return null;
    }).filter((e) => e !== null) as number[][]).flat();
        ...
        for (const stageIndex of stages) {
            ...
            for (let i = 0; i < questions.length; i++) {
                ...
                if ('required' in element && element.required) {
                    if (!Object.prototype.hasOwnProperty.call(values, element.id ?? `${stageIndex}.${i}`)) {
    ...
    const convertStageToElements = (stages: StageType, stage: number, index: number) => {
        switch (stages.type) {
            case 'number':
                return (<NumberPart
                    {...stages}
                    key={`number.${stage}.${index}`}
                    onNumberChange={(v) => setValues(
                        (o) => ({...o, [stages.id ?? `${stage}.${index}`]: v}),
                    )}
                />)
            case 'text':
                return (<TextPart
            ...
            case 'select':
                return (<SelectPart
                    {...stages}
                    key={`select.${stage}.${index}`}
                    onSelectChange={(v) => setValues(
                        (o) => ({...o, [stages.id ?? `${stage}.${index}`]: typeof (v) === 'string' ? v : v.value}),
                    )}
                />)
            case 'checkbox':
                return (<CheckboxPart
    ...
    return (
        <div className={styles.form}>
            {status ? <div className={styles.error} ref={ref}>{status}</div> : undefined}
            {elements}
            <Button
                color={Theme.BLUE}
                text='Submit'
                ...
                onClick={() => {
                    ...
                    if (validity === true) {
                        ...
                        setStatus(`${validity} - please correct the issues and submit again`);
                    }
                }}
            />
        </div>
...
export const FormDemo: React.FunctionComponent = () => {
    ...
    if (roles.includes("ops") || roles.includes("admin")) editor = (
