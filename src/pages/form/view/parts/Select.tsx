...
import apiInstance, { UEMSResponse } from '../../../../utilities/APIPackageGen';
import { NotificationContext } from '../../../../context/NotificationContext';
import { UIUtilities } from '../../../../utilities/UIUtilities';

export const OptionValidator = zod.string().or(
	zod.object({
		text: zod.string(),
		value: zod.string(),
	})
...
const SelectReferenceValidator = BasePart.extend({
	type: zod.literal('select'),
	value: zod.string().or(OptionValidator).optional(),
	reference: zod.enum(['venue', 'event', 'state', 'ents', 'topic']).or(
		zod.object({
...
export const SelectValidator = SelectOptionValidator.or(
	SelectReferenceValidator
);

type Props = { onSelectChange?: (value: Option) => void };
export type SelectOptionConfiguration = zod.infer<typeof SelectOptionValidator>;
export type SelectReferenceConfiguration = zod.infer<
	typeof SelectReferenceValidator
>;
export type SelectConfiguration = zod.infer<typeof SelectValidator>;
export type SelectProps = SelectConfiguration & Props;

const SelectOptionPart: React.FunctionComponent<
	SelectOptionConfiguration & Props
> = (props) => {
...
export const SelectEditPart: React.FunctionComponent<
	SelectConfiguration & {
...
> = (props) => {
	const [mode, setMode] = useState<'option' | 'reference'>(
		'reference' in props ? 'reference' : 'option'
	);
		...
		>
			<div style={{ display: 'flex', alignItems: 'self' }}>
				...
				<input
					...
					onChange={() =>
						setMode(referenceCheckbox.current?.checked ? 'reference' : 'option')
					}
				/>
			</div>
			{element}
...
const SelectReferencePart: React.FunctionComponent<
	SelectReferenceConfiguration & Props
> = (props) => {
	const [loadingState, setLoadingState] = useState<
		undefined | 'started' | 'loaded' | string
	>(undefined);
	...
	useEffect(() => {
		...
		let queryResponse: Promise<UEMSResponse<{ id: string; name: string }[]>>;
		switch (target) {
			case 'state':
				queryResponse = apiInstance.states().get({});
				break;
			case 'event':
				queryResponse = apiInstance.events().get({});
				break;
			case 'venue':
				queryResponse = apiInstance.venues().get({});
				break;
			case 'ents':
				queryResponse = apiInstance.ents().get({});
				break;
			case 'topic':
				queryResponse = apiInstance.topics().get({});
				break;
		}
	...
	return <div>Failed to load: {loadingState}</div>;
};
