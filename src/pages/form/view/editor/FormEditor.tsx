...
export const makeBlankConfig = (type: StageType['type'], existing?: BasePartType): StageType => {
    switch (type) {
        case 'number':
            return {
                type: 'number',
...
const FormEditor: React.FunctionComponent<Form & { update: undefined | ((form: Form) => void), done: () => void }> = (props) => {
    ...
    const rebuildAndUpdate = (steps: StageType[], selectedStage?: number) => {
        ...
        const stage = 'title' in originalStage
    ...
    if (selectedEntry !== -1 && selectedState !== -1) {
        right = (<div style={{padding: '20px'}}>
            <FormStageEditor key={`s${selectedState}e${selectedEntry}`}
        ...
        right = (<StagePropertyEditor key={`stage.${selectedState}`}
                                      form={props}
                                      index={selectedState}
    ...
    return (
        <DndProvider backend={HTML5Backend}>
            <Button color={Theme.PURPLE} text={'Done'} style={{float: 'right'}} onClick={props.done}/>
            <div className={styles.horizontal}>
                <div className={styles.left}>
                    {props.stages.map((e, i) => (<StageSet
                        key={`stage.${i}`}
                        onRemoveStage={(idx) => {
                        ...
                        selectedStage={selectedState === i && selectedEntry !== -1 ? selectedEntry : -1}
                        set={e}
                        idx={i}
                        update={(s) => {
