...
const FlowConditionEditor: React.FunctionComponent<FlowConditionEditorProps> = (
	props
) => {
	...
	const convert = (c?: {
		...
		c ? { value: c.value, text: c.label } : undefined;

	let matched: { stage: number; index: number; entry: StageType } | undefined =
		undefined;
	if (flow !== undefined) {
		if (/\d+.\d+/.test(flow.target)) {
			...
			let temp = questions.find(
				(e) => e.additional.stage === stage && e.additional.index === index
			);
	...
	const [selectedQuestion, setSelectedQuestion] = useState<
		{ stage: number; index: number; entry: StageType } | undefined
	>(matched);
	const [selectedOperator, setSelectedOperator] = useState<
		KeyValueOption | undefined
	>(convert(operators.find((e) => e.value === flow?.type)));
	const [selectedComparison, setSelectedComparison] = useState<string | number>(
		flow?.value
	);
	...
	return (
		<div>
			<Select
				placeholder={'When question'}
				name={'question'}
				options={questions}
				initialOption={questions.find(
					(e) =>
						e.additional.stage === selectedQuestion?.stage &&
						e.additional.index === selectedQuestion?.index
				)}
			...
			<Select
				...
				key={
					selectedQuestion
						? `${selectedQuestion.stage}:${selectedQuestion.entry}`
						: 'unset'
				}
				name={'op'}
				...
				options={
					operators
						.filter((e) =>
							e.requirement.includes(selectedQuestion?.entry?.type as string)
						)
						.map(convert)
			...
			/>
			<TextField
				onChange={(v: any) => setSelectedComparison(v)}
				...
				onClick={() => {
					if (
						...
						onSave({
							...
							target:
								(selectedQuestion.entry as any)?.id ??
								`${selectedQuestion.stage}.${selectedQuestion.index}`,
						});
					}
				}}
			/>
