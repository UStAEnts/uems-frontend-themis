...
import {
    ...
    faHashtag, faHeading,
    faList,
    faPencilAlt, faTimesCircle
} from "@fortawesome/free-solid-svg-icons";
import {classes} from "../../../../utilities/UIUtilities";
import styles from "../FormEditor.module.scss";

type StageElementProps = {
    stage: StageType, move: (drag: number, hover: number) => void, index: number,
    onClick: () => void,
    selected: boolean,
    onRemove: () => void,
};
const StageElement: React.FunctionComponent<StageElementProps> = ({
    ...
    // drag(drop(ref))
    return (
        <div className={classes(styles.stageEntry, styles.horizontal, selected ? styles.selected : undefined)}
             onClick={(e) => {
                 // @ts-ignore
        ...
        >
            <div className={classes(styles.left, styles.centered)}>
                {icon}
            </div>
            <div className={styles.right}>
                ...
                {'id' in stage
                    ? (<div className={classes(styles.additional, styles.id)}>#{stage.id}</div>)
                    : undefined}
            </div>
