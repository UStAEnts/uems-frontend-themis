...
const StagePropertyEditor: React.FunctionComponent<StagePropertyEditorProps> = (props) => {
    ...
    const questionOptions: KeyValueOption[] = form.stages
        .map((e, si) => getQuestions(e).map((entry, ei) => ({stage: si, index: ei, entry})))
        .flat()
        .filter((e) => e.entry.type !== 'title')
        .map((question) => {
            if (question.entry.type === 'title') return;
            return ({
                text: `(${question.entry.id ?? `${question.stage + 1}.${question.index}`}) ${question.entry.prompt}`,
    ...
    return (
        <div style={{padding: '20px'}}>
            ...
            <div>
                <input type='checkbox'
                       checked={form.flow.some((e) => e === index)}
                />
            ...
            <div>
                ...
                {
                    showAddNew
                        ? <FlowConditionEditor onSave={(flow) => {
                            const flows = [...form.flow];

                            // Clean up unconditional state
                            const numericalIndex = flows.findIndex((e) => typeof (e) === 'number' && e === index);
                            if (numericalIndex !== -1) flows.splice(numericalIndex);

                            // Then find the existing entry if it exists
                            const entryIndex = flows.findIndex((e) => typeof (e) !== 'number' && e.stages === index);
                            if (entryIndex === -1) {
                            ...
                            setShowAddNew(false);
                        }} questions={questionOptions} stage={index}/>
                        : undefined
                }
                {
                    showAddNew
                        ? undefined
                        : <Button color={Theme.PURPLE} text={'Add new'} onClick={() => setShowAddNew(true)}/>
                }
            </div>
        </div>
    );
