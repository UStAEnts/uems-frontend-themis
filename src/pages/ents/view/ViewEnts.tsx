...
import apiInstance, { EntState, EventList } from "../../../utilities/APIPackageGen";
...
class ViewEntsClass extends FallibleReactComponent<ViewEntsPropsType, ViewEntsStateType> {
	...
	private patch = (update: EntState) => {
		console.log(update);
		apiInstance.ents().id(this.props.match.params.id).patch(update).then(() => {
			// To fix some typing
		...
		if (this.state.ents) {
			return (
				<EventOrCommentRelatedView
					...
					delete={ {
						redirect: '/ents',
						onDelete: () => UIUtilities.deleteWith409Support(() => apiInstance.ents().id(this.props.match.params.id).delete()),
					} }
				/>
			);
		}
