...
class CreateEntsClass extends React.Component<
	CreateEntsPropsType,
	CreateEntsStateType
> {
	...
	private save = () => {
		for (const key of [
			...
			'color',
			'icon',
		] as (keyof CreateEntsStateType['ents'])[]) {
		...
		return (
			<div className={`create-event ${this.props.isPage ? 'page' : ''}`}>
				...
				>
					<IconBox
						icon={
							(this.state.ents.icon
								? ['fas', this.state.ents.icon.identifier]
