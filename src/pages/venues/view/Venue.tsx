...
import apiInstance, { EventList, PATCHVenuesIdBody, Venue as UEMSVenue } from "../../../utilities/APIPackageGen";
...
class VenueClass extends FallibleReactComponent<ViewVenueProps, State> {
	...
	private patch = (update: PATCHVenuesIdBody) => {
		UIUtilities.load(this.props, apiInstance.venues().id(this.props.venue.id).patch(update), (e) => `Failed to update your venue! ${ e }`)
			.data(() => {
				// To fix some typing
	...
	realRender() {
		return (
			<EventOrCommentRelatedView
				...
				delete={ {
					redirect: '/venues',
					onDelete: () => UIUtilities.deleteWith409Support(() => apiInstance.venues().id(this.props.venue.id).delete()),
				} }
			/>
		);
	}
