...
class CreateFileClass extends React.Component<
	CreateFilePropsType,
	CreateFileStateType
> {
	...
	private save = () => {
		...
		for (const key of [
			...
			'file',
			'type',
		] as (keyof CreateFileStateType['file'])[]) {
		...
		UIUtilities.loadUnwrapped(
			this.props,
			apiInstance.files().post({
			...
			(e) =>
				`Failed to create your file! The metadata was not created and no file was uploaded: ${e}`
		).data((id) => {
	...
	render() {
		return (
			<div className={`create-event ${this.props.isPage ? 'page' : ''}`}>
				...
				<FileUpload
					onFileSelect={failEarlyStateSet(
						this.state,
						this.setState.bind(this),
						'file',
						'file'
