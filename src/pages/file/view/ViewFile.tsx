...
class ViewFileClass extends FallibleReactComponent<
	ViewFilePropsType,
	ViewFileStateType
> {
	...
	private renderEvent =
		(iconType: 'delete' | 'add') => (event: EventList[number]) =>
			(
	...
	private generateEventList = () => {
		if (this.state.events === undefined) {
			return <Loader color={Theme.TEAL} type={this.loaderType} />;
		}

		const attachedEvents = this.state.attached
			? this.state.attached.map((e) => e.id)
			: [];
		let events: EventList = this.state.events.filter(
			(e) => !attachedEvents.includes(e.id)
		);
		...
		return (
			<GenericList
				...
				onClick={(entry: EventList[number]) => {
					UIUtilities.load(
						this.props,
						apiInstance.events().id(entry.id).files().post({
							fileID: this.props.match.params.id,
						}),
						(e) => `Failed to link file! ${e}`
					).data(() => {
						this.setState((old) => ({
							...old,
		...
		if (!file) {
			return (
				<div className="view-file loading">
					<Loader color={Theme.TEAL} type={this.loaderType} />
				</div>
			);
		}
