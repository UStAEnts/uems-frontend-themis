...
class CreateStateClass extends React.Component<
	CreateStatePropsType,
	CreateStateStateType
> {
	...
	private save = () => {
		for (const key of [
			'name',
			'color',
		] as (keyof CreateStateStateType['state'])[]) {
					...
					<IconBox
						icon={
							(this.state.state.icon
								? ['fas', this.state.state.icon.identifier]
...
export const CreateState = withRouter(
	withNotificationContext(CreateStateClass)
);
