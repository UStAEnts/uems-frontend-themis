...
class CreateTopicClass extends React.Component<CreateTopicPropsType, CreateTopicStateType> {
	...
	private save = () => {
		...
		UIUtilities.load(this.props, apiInstance.topics().post({
			color: this.state.topic.color ?? '#000000',
			description: this.state.topic.description ?? 'No description provided...',
			...
			.data((id) => {
				...
				failEarlyStateSet(this.state, this.setState.bind(this), 'ui', 'redirect')(`/topics/${ id[0] }`);
			})
	}
